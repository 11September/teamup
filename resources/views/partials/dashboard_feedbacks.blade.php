            ...
            <div class="card">
                <div class="card-body bg1">
                    <h4 class="header-title text-white">
                        <a class="allFeedbacks" href="{{ action('Admin\FeedbacksController@index') }}">Client
                            Feadbacks</a>
                    </h4>
                                ...
                                @foreach($feedbacks as $feedback)
                                    ...
                                    <div class="tst-item">
                                        <div class="tst-wrapper-item">
                                            <div class="tstu-img">
                                                <a href="{{ action('Admin\UsersController@edit', $feedback->user->id) }}">
                                                    @if($feedback->user->avatar)
                                                        <img src="{{ asset($feedback->user->avatar) }}"
                                                             alt="author image">
                                                    @else
                                            ...
                                            <div class="tstu-content">
                                                <h4 class="tstu-name">
                                                    <a class="mailTo"
                                                       href="{{ action('Admin\UsersController@edit', $feedback->user->id) }}">{{ $feedback->user->getFullnameAttribute() }}</a>
                                                </h4>
                                                <span class="profsn"><a class="mailTo"
                                                                        href="mailto:{{ $feedback->user->email }}">{{ $feedback->user->email }}</a></span>
